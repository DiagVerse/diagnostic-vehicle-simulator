...
**MVP-2 delivered** (`feat/mvp-ui-simulate`): the browser demo plus **effective** UDS timing. A Simulate tab loads a CAN log, lists the reconstructed ECUs with confidence badges, and drives them by CAN address (physical or broadcast). `EcuTiming` gained P4Server_max and the operator's knobs (response delay, forced ResponsePending count, drop-final-response) with validation; `ecu::schedule` turns a request into a `ResponsePlan` (byte strings + millisecond offsets), and the API executes it — so a delay past P2 really does emit NRC 0x78 then the answer. Endpoints: `GET/PUT /simulation/ecus/:requestCanIdHex/timing`. See ADR 0005. Next: MVP-3 (ISO-TP segmentation + SLCAN bridge), MVP-4 (demo polish).

**MVP-2b delivered** (`feat/vehicle-builder-and-topology`): a vehicle can be built by hand (`POST /simulation/vehicle`, `POST /simulation/ecus`, `DELETE`/rename per ECU) as well as reconstructed; any ECU's answers can be edited via **response overrides** (`GET/PUT /simulation/ecus/:id/overrides`) — pattern + per-byte mask + echo spans, substitute or suppress, most-specific-first; and `GET /simulation/topology` plus a Topology tab draw the vehicle. See ADR 0006. Note the constraint it documents: declaring a service supported does not implement it — the UDS plugin answers 0x10/0x11/0x19/0x22/0x27/0x31/0x3E and an override is the only way to get a positive response out of anything else.
...
**DoIP delivered** (ADRs 0015–0019): `crates/doip` (ISO 13400-2 codec + connection state machine, no sockets), `crates/doip-server` (UDP discovery + TCP entity; `/doip/status|start|stop`), routing by logical address (`EcuKey::Can|DoIp` — an ECU reachable both ways is **one** `VirtualEcu`), `VehicleIdentity` (VIN/EID/GID), ECU **handles** so a DoIP-only ECU is addressable (`doip-1234` alongside a CAN id), and `crates/pcap` + `reconstruct::doip` — **a fourth vehicle source: import a pcap/pcapng DoIP capture**. `POST /simulation/pcap`; `dvsim reconstruct` sniffs the file. `reconstruct::behaviour` is now shared by both the CAN and DoIP pipelines. Not built, deliberately: power-up announcement, alive-check arbitration (codes 0x01/0x03 are *stricter* than the standard, never looser), TLS on 3496, authentication/confirmation states.

**DoIP conformance pass** (ADR 0021): an ISO 13400-2:2019 audit of the above, applied. Functional group addresses (Table 13, `0xE000`–`0xEFFF`) are routed as a broadcast to the whole vehicle instead of getting NACK `0x03`, with REQ 7.DoIP-072 AL's length limit refusing one that would have to cross a CAN sub-network; the two inactivity timers are now genuinely different (the general one resets on any data, the initial one on nothing at all, and an elapsed timer actually closes the socket); advertised socket capacity is the enforced capacity; an ECU's response plan keeps its timing over DoIP, so a response delay or forced `0x78` is observable; the vehicle identification response waits `A_DoIP_Announce_Wait`; and the capture reader frames past payload types it cannot decode rather than abandoning the stream. Two socket-level harnesses — `scripts/doip-loop-test.py` (capture-driven, 6 ECUs) and `scripts/doip-vehicle-test.py` (a 47-ECU simfile, 46 of them DoIP-addressable) — drive the entity as a tester does and assert the refusals, not just the answers.

**Flow control, line speed and security state** (ADR 0022): the decisions from the hardware sessions, recorded. ISO-TP BlockSize defaults to **1** rather than the standard's 0 — over an SLCAN dongle a BlockSize of 0 loses the middle of a long request, which is a silent wrong answer rather than a slow one; the bridge (`LinkCapacity`/`SafeSeparationTime`) refuses to advertise a pacing the link cannot honour and overrides an ECU that asks for none; the host-to-adapter baud rate is **probed at runtime** (`bridge::probe`, fastest first, reply checked for shape) and `BaudRateSource` says whether the number in use was measured or guessed; `SecurityKeyPolicy` gives each level compare / acceptAny / refuseWith, with reconstruction defaulting to **acceptAny** because a capture shows a key but never the algorithm; and entering the default session relocks security **and drops any outstanding seed** (ISO 14229-1 §10.3), the second half being what otherwise produces an NRC 0x24 one cycle later. `PUT /simulation/flow-control` applies one BlockSize/STmin to every ECU at once — only those two fields, so deliberate per-ECU fault injection survives it. `POST /hw/line-speed` commands the adapter's own UART faster via the SLCAN `U` command — explicit, never automatic (the setting outlives a power cycle on several firmwares), and the link is put back if the adapter does not answer at the new speed.

**PDX / ODX — a fifth source** (`scripts/pdx-to-simfile.py`): a PDX archive (or a zip of per-ECU PDX files) converts to a simulation file, which the engine already loads. Deliberately a converter rather than a Rust crate: ODX spreads an ECU's services across shared layers via PARENT-REF/IMPORT-REF — in the reference delivery an ECU declares 7 services and resolves to 38 — and re-implementing that inheritance would produce a worse parser than `odxtools` while putting thousands of lines of XML handling inside the engine. The simulation file *is* the Unified Vehicle Model in document form, so this is a populator in exactly the sense README §23 means. **What ODX cannot give**: DID *values*. A specification states what an ECU answers and the shape of it, never the value, so every DID gets a visibly-labelled placeholder of the declared length (hex when the field is too short to carry a label) and the vehicle name says so. Variants are real: a delivery describes every build a platform offers, and two files for one address are alternatives — one is kept per address, the rest reported, `--prefer` picks between them. `scripts/test_pdx_to_simfile.py` covers the readers without needing `odxtools`; CI runs it. **In the app**: a *From a PDX* source tab uploads **one file, several at once, or a zip** to `POST /simulation/pdx` (multipart), which runs the converter and loads what it writes — so the engine keeps no ODX code and gains only an optional runtime need for Python + odxtools, reported plainly when either is missing. The converter's report (variants set aside, files parsed leniently) is carried back and shown, because a conversion *chooses* ECUs out of a delivery and the operator has to be able to disagree with the choice.

**DoIP is visible and bindable** (`feat/see-doip-traffic-and-choose-an-interface`): a `DoIpObserver` port in `doip-server` mirrors the CAN side's `FrameObserver`, so every DoIP message crossing the wire reaches the traffic monitor — payload type named, peer address shown, UDP and TCP separated. It rides the same `frames` switch as CAN frames, because a flash over Ethernet floods a browser exactly as one over CAN does. `GET /doip/interfaces` lists this machine's IPv4 addresses **with their subnets** and the DoIP tab turns them into buttons that fill the bind field, flagging loopback ("this machine only") and link-local ("no DHCP") — choosing wrongly is otherwise invisible until a tester on another subnet gets no answer. Typing the tester's address marks each interface *reaches your tester* or *cannot reach it*, which is the question an address alone cannot answer: `192.168.1.8` and `192.168.11.10` look alike and cannot talk. **The entity announces the gateway**, not the lowest logical address — on a real vehicle the DoIP entity *is* the gateway, and announcing whichever ECU sorted first meant a 50-ECU vehicle introduced itself as an arbitrary node. A source that states no topology (a PDX delivery, a capture) marks no gateway, so the fallback is the lowest address, said out loud in the log, and the DoIP tab's *Announce as* selector names one explicitly.

**Naming the gateway** (`PUT /simulation/gateway`, and a *make gateway* action per ECU): one call marks an ECU as the vehicle's front door and puts every other ECU on a link behind it. A gateway forwards onto *networks*, so saying this by hand is a network declaration plus one placement call per ECU — fifty on a real delivery — which is why vehicles that genuinely have a gateway were being simulated flat. Flat costs three real things: the DoIP entity address falls back to the lowest logical address rather than the gateway a tester actually meets; switching the gateway off silences one ECU instead of the whole vehicle behind it; and the topology draws peers. Re-callable, and naming a different ECU moves the role rather than adding a second gateway. Note what it does **not** change: addresses, names, or how a tester reaches anything — a locked gateway is still one TCP socket to the entity with every node addressed through it, which is what the DoIP routing already did.

**D-PDU: decided not to build.** ISO 22900-2 is a C API into a DLL on the tester PC — there is no D-PDU wire protocol (clause 8.1.1 leaves any tester↔VCI link vendor-proprietary). Annex K.2.4.1 "MVCI use case 2" is our case: a conformant DoIP server is sufficient and needs zero D-PDU code from us.
